...
function handleConfirm() {
    ...
    if (currentTipIndex < tipsData.length - 1) {
        ...
        Progress.completeSubMode("soft_skills", "soft_lesson");
    }
}
...
async function loadData() {
    try {
        const response = await fetch('../db/soft_skills_tips.json');
        ...
        console.error('Error loading soft_skills_tips data:', error);
    }
}
